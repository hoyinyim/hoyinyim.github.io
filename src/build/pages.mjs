...
import { miniGlyph, pageGlyph, sectionGlyph } from './glyphs.mjs';
...
export function researchPage({ research, siteGlyphs }) {
  const focusGlyphs = ['study-oracle', 'book-oracle', 'speech-oracle'];
  return `<header class="research-opening axis-grid">${pageGlyph(siteGlyphs, 'study-oracle', 'research')}<p class="overline">研究／中國思想</p><h1>研究</h1><div><strong>${research.current.length} 項研究重點</strong><p>${research.tags.map(escapeHtml).join('／')}</p></div></header>
  <section class="research-focus-list" aria-labelledby="research-focus-title"><h2 id="research-focus-title" class="visually-hidden">目前研究方向</h2>${research.current.map((item, index) => `<article class="focus-${index + 1} axis-grid" data-glyph-focus>${sectionGlyph(siteGlyphs, focusGlyphs[index], `research-focus-${index + 1}`)}<p class="focus-number">0${index + 1}</p><p class="overline">目前研究重點</p><h3>${escapeHtml(item.title)}</h3><p class="focus-copy">${escapeHtml(item.description)}</p></article>`).join('')}</section>
  <section class="research-matrix-section" aria-labelledby="research-matrix-title">${sectionGlyph(siteGlyphs, 'study-oracle', 'research-matrix')}<header class="section-index"><p>研究矩陣</p><h2 id="research-matrix-title">研究專長</h2></header><div class="expertise-matrix">${research.expertise.map((item) => `<article><h3>${escapeHtml(item.title)}</h3><p>${escapeHtml(item.description)}</p></article>`).join('')}</div></section>`;
}
...
export function journalPage({ publications, pageIntros, siteGlyphs }) {
  ...
  const entries = years.map((year) => `<section class="publication-year" id="year-${year}" data-publication-year><header>${miniGlyph(siteGlyphs, 'book-oracle', 'year-index')}<h2>${year}</h2><p>${groups[year].length} 筆</p></header><ol>${groups[year].map((pub) => `<li class="publication-entry" data-publication data-filters="${escapeHtml(pub.filter.join(' '))}"><article><p class="publication-venue">${escapeHtml(pub.venue)}</p><h3>${escapeHtml(pub.title)}</h3><p class="publication-meta">${escapeHtml(pub.meta)}</p>${tags(pub.tags)}<div class="entry-actions">${pub.link ? `<a href="${pub.link}" ${externalAttrs}>電子全文 ↗</a>` : '<span>全文連結待補</span>'}<button type="button" data-copy="${escapeHtml(pub.cite)}">複製引用</button></div><details><summary>引用格式</summary><p>${escapeHtml(pub.cite)}</p></details></article></li>`).join('')}</ol></section>`).join('');
  return `<header class="journal-opening axis-grid">${pageGlyph(siteGlyphs, 'book-oracle', 'journal')}<p class="overline">期刊論文／書目檔案</p><h1>期刊論文</h1><div><strong>${publications.length} 筆著作</strong><p>${escapeHtml(pageIntros.journal.lead)}</p></div></header><div class="archive-toolbar" id="journal-top"><p>${miniGlyph(siteGlyphs, 'book-oracle', 'filter')}篩選</p><div class="filter-strip" role="group" aria-label="篩選期刊論文">${filters.map((filter, index) => `<button type="button" data-publication-filter="${filter.id}" aria-pressed="${index === 0}">${escapeHtml(filter.label)}</button>`).join('')}</div><p><span data-publication-count>${publications.length}</span> 筆書目</p></div><div class="publication-archive" data-publication-archive>${entries}</div>`;
}

export function conferencePage({ conferences, pageIntros, siteGlyphs }) {
  const groups = groupBy(conferences.presentations, 'year');
  return `<header class="conference-opening axis-grid">${pageGlyph(siteGlyphs, 'speech-oracle', 'conference')}<p class="overline">研討會論文／學術發表</p><h1>研討會論文</h1><div><strong>${conferences.published.length} 篇已出版／${conferences.presentations.length} 次發表</strong><p>${escapeHtml(pageIntros.conference.lead)}</p></div></header>
  <section class="conference-published" aria-labelledby="published-title">${sectionGlyph(siteGlyphs, 'book-oracle', 'conference-published')}<header class="section-index"><p>已出版論文集</p><h2 id="published-title">研討會論文集（已出版）</h2></header>${conferences.published.map((item) => `<article><p class="publication-venue">${escapeHtml(item.venue)}</p><h3>${escapeHtml(item.title)}</h3><p>${escapeHtml(item.meta)}</p><a href="${item.link}" ${externalAttrs}>電子全文 ↗</a></article>`).join('')}</section>
  <section class="conference-ledger" aria-labelledby="presentations-title">${sectionGlyph(siteGlyphs, 'journey-oracle', 'conference-timeline')}<header class="section-index"><p>學術發表</p><h2 id="presentations-title">研討會發表</h2></header>${Object.entries(groups).map(([year, items]) => `<section class="conference-year">${miniGlyph(siteGlyphs, 'journey-oracle', 'conference-year')}<h3>${escapeHtml(year)}</h3><ol>${items.map((item) => `<li><h4>${escapeHtml(item.title)}</h4><p>${escapeHtml(item.meta)}</p></li>`).join('')}</ol></section>`).join('')}</section>`;
}

export function translationsPage({ translations, pageIntros, siteGlyphs }) {
  const { translation, publicWriting } = translations;
  const latest = publicWriting.slice(0, 3);
  const archive = publicWriting.slice(3);
  return `<header class="translations-opening axis-grid">${pageGlyph(siteGlyphs, 'speech-oracle', 'translations')}<p class="overline">譯著／公共哲學</p><h1>譯著／<br>哲學普及作品</h1><p>${escapeHtml(pageIntros.translations.lead)}</p></header>
  <section class="translation-feature" aria-labelledby="translation-title">${sectionGlyph(siteGlyphs, 'book-oracle', 'translation-book')}<figure><img src="${translation.cover}" width="500" height="700" loading="lazy" decoding="async" alt="${escapeHtml(translation.coverAlt)}"></figure><div><p class="overline">書籍翻譯／2024</p><h2 id="translation-title">${escapeHtml(translation.title)}</h2><p class="original-title">${escapeHtml(translation.originalTitle)}</p><dl><div><dt>作者</dt><dd>${escapeHtml(translation.author)}</dd></div><div><dt>譯者</dt><dd>${escapeHtml(translation.translator)}</dd></div><div><dt>出版社</dt><dd>${escapeHtml(translation.publisher)}</dd></div><div><dt>出版日</dt><dd>${escapeHtml(translation.publicationDate)}</dd></div></dl><p>${escapeHtml(translation.description)}</p><a class="axis-link" href="${translation.link}" ${externalAttrs}>前往出版社 ↗</a></div></section>
  <section class="latest-writing" aria-labelledby="latest-writing-title">${sectionGlyph(siteGlyphs, 'speech-oracle', 'public-writing')}<header class="section-index"><p>最新公共寫作</p><h2 id="latest-writing-title">最新公共寫作</h2></header><div>${latest.map((item) => `<article><p>${escapeHtml(item.source)}</p><h3><a href="${item.link}" ${externalAttrs}>${escapeHtml(item.title)}</a></h3><p>${escapeHtml(item.description)}</p></article>`).join('')}</div></section>
...
export function certificatesPage({ credentials, pageIntros, siteGlyphs }) {
  return `<header class="credentials-opening axis-grid">${pageGlyph(siteGlyphs, 'journey-oracle', 'credentials')}<p class="overline">證照／榮譽</p><h1>證照、證書<br>與獎項</h1><p>${escapeHtml(pageIntros.certificates.lead)}</p></header>
  <section class="credential-ledger" aria-labelledby="credentials-title">${sectionGlyph(siteGlyphs, 'book-oracle', 'credentials-record')}<header class="section-index"><p>專業資格</p><h2 id="credentials-title">專業證照與證書</h2></header><ol>${credentials.credentials.map((item) => `<li><p>${escapeHtml(item.category)}</p><h3>${escapeHtml(item.title)}</h3><p>${escapeHtml(item.issuer)}</p></li>`).join('')}</ol></section>
  <section class="award-timeline" aria-labelledby="awards-title">${sectionGlyph(siteGlyphs, 'journey-oracle', 'recognition-timeline')}<header class="section-index"><p>學術榮譽</p><h2 id="awards-title">學業與研究獎項</h2></header><ol>${credentials.awards.map((item) => `<li><time>${escapeHtml(item.year)}</time><div><h3>${escapeHtml(item.title)}</h3><p>${escapeHtml(item.description)}</p></div></li>`).join('')}</ol><p class="archive-note">${escapeHtml(pageIntros.certificates.closing)}</p></section>`;
}

export function teachingPage({ experience, siteGlyphs }) {
  const [teaching, ...practice] = experience;
  return `<header class="teaching-opening axis-grid">${pageGlyph(siteGlyphs, 'teach-oracle', 'teaching')}<p class="overline">教學／學術實踐</p><h1>教學</h1><p>教學工作與學術、翻譯及公共實踐紀錄。</p></header><section class="teaching-primary axis-grid" aria-labelledby="teaching-title">${sectionGlyph(siteGlyphs, 'study-oracle', 'teaching-study')}<p class="overline">教學經驗</p><h2 id="teaching-title">實際教學經驗</h2><article><p>${escapeHtml(teaching.text)}</p></article></section><section class="practice-ledger" aria-labelledby="practice-title">${sectionGlyph(siteGlyphs, 'journey-oracle', 'practice-path')}<header class="section-index"><p>學術與專業實踐</p><h2 id="practice-title">學術與專業實踐</h2></header><ol>${practice.map((item) => `<li><p>${escapeHtml(item.text)}</p></li>`).join('')}</ol><a class="axis-link" href="certificates.html">查看專業證照與獎項 ↗</a></section>`;
}

const cvList = (items, render) => `<ol class="cv-list">${items.map(render).join('')}</ol>`;
export function cvPage({ profile, education, experience, research, publications, conferences, translations, credentials, siteGlyphs }) {
  ...
  return `<header class="cv-header">${pageGlyph(siteGlyphs, 'journey-oracle', 'cv')}<p class="overline">個人履歷</p><h1>${escapeHtml(profile.nameZh)} <span>${escapeHtml(profile.nameEn)}</span></h1><div><a href="mailto:${escapeHtml(profile.email)}">${escapeHtml(profile.email)}</a><a href="downloads/yim-ho-yin-cv.pdf" download>下載 PDF</a><button type="button" data-print-cv>列印／儲存 PDF</button></div></header><div class="cv-layout"><nav class="cv-index" aria-label="履歷頁內索引">${miniGlyph(siteGlyphs, 'journey-oracle', 'cv-index')}${sections.map(([id, label]) => `<a href="#${id}">${label}</a>`).join('')}</nav><div class="cv-document">
...
export function contactPage({ profile, siteGlyphs }) {
  return `<section class="contact-scene axis-grid" aria-labelledby="contact-title">${pageGlyph(siteGlyphs, 'speech-oracle', 'contact')}<p class="overline">聯絡／學術通信</p><h1 id="contact-title">聯絡</h1><div class="contact-identity"><strong>${escapeHtml(profile.nameZh)}／${escapeHtml(profile.nameEn)}</strong><p>${escapeHtml(profile.roleZh)}</p><p>${escapeHtml(profile.institutionZh)}</p></div><a class="contact-email" href="mailto:${escapeHtml(profile.email)}">${escapeHtml(profile.email)}</a><div class="contact-actions"><a href="mailto:${escapeHtml(profile.email)}">撰寫郵件</a><button type="button" data-copy="${escapeHtml(profile.email)}" data-glyph-copy>複製 Email</button></div></section>`;
}
